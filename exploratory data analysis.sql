...
select industry, left(`date`,7) month, total_laid_off, sum(total_laid_off) over(partition by industry order by left(`date`,7) asc) accumulative
from layoffs_staging2
where industry is not null
and total_laid_off is not null;

select industry, year(`date`) year, sum(total_laid_off)
from layoffs_staging2
where total_laid_off is not null
...
select industry, `year`, sum, sum(`sum`) over(partition by industry order by `year`) accumulative
